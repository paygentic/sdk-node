 ...
 */
export function entitlementsGrantsList(
  client: PaygenticCore,
  request: operations.ListEntitlementGrantsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.ListEntitlementGrantsResponse,
    | errors.BadRequest
    | errors.ErrorT
    | PaygenticError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListEntitlementGrantsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v1/entitlements/{entitlementId}/grants")(
    pathParams,
  );

  const query = encodeFormQuery({
    "includeVoided": payload.includeVoided,
    "limit": payload.limit,
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ListEntitlementGrantsResponse$inboundSchema),
