 ...
 */
export function billingSchedulesReplaceScheduleIntervals(
  client: PaygenticCore,
  request: operations.ReplaceScheduleIntervalsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.ReplaceScheduleIntervalsResponse,
    | errors.BadRequest
    | errors.ErrorT
    | PaygenticError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ReplaceScheduleIntervalsRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.ReplaceScheduleIntervalsRequest, {
    explode: true,
  });
  ...
  const path = pathToFunc("/v0/billingSchedules/{id}/intervals")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ReplaceScheduleIntervalsResponse$inboundSchema),
