 ...
 */
export function billingSchedulesUpdateBillingSchedule(
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.UpdateBillingScheduleRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.UpdateBillingScheduleRequest, {
    explode: true,
  });
  ...
  const path = pathToFunc("/v0/billingSchedules/{id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.SchemasBillingSchedule$inboundSchema),
