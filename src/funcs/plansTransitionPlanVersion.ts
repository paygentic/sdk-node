 ...
 */
export function plansTransitionPlanVersion(
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.TransitionPlanVersionRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.TransitionPlanVersionRequest, {
    explode: true,
  });
  ...
  const pathParams = {
    ...
    versionNumber: encodeSimple("versionNumber", payload.versionNumber, {
      explode: false,
      charEncoding: "percent",
    }),
  };
  const path = pathToFunc("/v0/plans/{id}/versions/{versionNumber}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.PlanVersion$inboundSchema),
