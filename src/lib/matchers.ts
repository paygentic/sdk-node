...
import { PaygenticDefaultError } from "../models/errors/paygenticdefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | PaygenticDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | PaygenticDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new PaygenticDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new PaygenticDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
